...
class AddComputingNodesToHAClusterStep(workflows.UpdateMembersStep):
    ...
    def contribute(self, data, context):
        if data:
            member_field_name = self.get_member_field_name('member')
            context['computing_nodes'] = data.get(member_field_name, [])
            context['detection'] = data.get(member_field_name, [])
        return context

class SelectDetectionAction(workflows.MembershipAction):
    def __init__(self, request, *args, **kwargs):
        super(SelectDetectionAction, self).__init__(request, *args, **kwargs)

        detection_list = [('1', 'Host Power'),
                          ('2', 'Host OS'),
                          ('3', 'Host Network'),
                          ('4', 'Host Service'),
                          ('5', 'VM Power'),
        ...
        field_name = self.get_member_field_name('member')
        self.fields[field_name] = forms.MultipleChoiceField(required=False)
        self.fields[field_name].choices = detection_list


    class Meta(object):
            name = _("Detections")

class SelectDetectionStep(workflows.UpdateMembersStep):
    action_class = SelectDetectionAction
    show_roles = False
    help_text = _("Select detection type that you want to protect the host. If no detections are selected, all detections will active by default.")
    available_list_title = _("All Detections")
    members_list_title = _("Selected Detections")
    no_members_text = _("No detection selected.")
    contributes = ("detection",)
...
class CreateHAClusterWorkflow(workflows.Workflow):
    ...
    default_steps = (SetHAClusterInfoStep, AddHostsToHAClusterStep, SelectDetectionStep)
    ...
    def handle(self, request, context):
        ...
        for detection in context_detection:
            detection_list.append(detection)
        result = server.create_cluster(name, node_list, detection_list)
...
class AddComputingNodeWorkflow(workflows.Workflow):
    ...
    default_steps = (AddComputingNodesToHAClusterStep,SelectDetectionStep)
    ...
    def handle(self, request, context): 
        ...
        for detection in context_detection:
            detection_list.append(detection)
        result = server.add_node(cluster_id,node_list,detection_list)
